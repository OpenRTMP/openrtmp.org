  ...
  <section style="padding-top: 0;">
    <div class="container docs-layout">
      ...
      <div class="docs-content">
        ...
        <p>Add <code>librtmp2</code> from <a href="https://crates.io/crates/librtmp2" target="_blank" rel="noopener">crates.io</a> to your <code>Cargo.toml</code>. While <code>librtmp2</code> remains on <code>0.x</code>, use a loose <code>0.x</code> range when you want regular updates and refresh the lockfile for security fixes (RTMPS/TLS via the default <code>tls</code> feature):</p>
        <pre><code>[dependencies.librtmp2]
version = "0"

cargo update -p librtmp2   # pull newest 0.x from crates.io</code></pre>
        <p>For reproducible builds, replace the loose range with the exact release you tested. Use the version shown on <a href="https://crates.io/crates/librtmp2" target="_blank" rel="noopener">crates.io</a> rather than copying a “current” version number from this page.</p>
...
CLUSTER_MEDIA_ADVERTISE_ADDR=10.0.0.1:1941</code></pre>
        ...
        <pre><code>docker network create openrtmp   # shared with the panel examples below
...
docker logs librtmp2-server   # copy API token from first-start output</code></pre>
        <p>Available tags: <code>latest</code>, <code>beta</code>, <code>alpha</code>, and pinned versions (for example a concrete release tag from GitHub Releases).</p>
        ...
        <pre><code>docker network create openrtmp   # skip if you created it above
...
docker run -d \
  ...
  ghcr.io/openrtmp/librtmp2-server-panel:latest</code></pre>
        ...
        <p>The panel repo's <code>compose.quickstart.yml</code> runs the same three services from the published images. Set secrets in <code>.env</code> <em>before</em> the first start so the server seeds the shared API token:</p>
...
docker compose -f compose.quickstart.yml up -d</code></pre>
        <p>The repo's default <code>docker-compose.yml</code> instead builds the server from a sibling <code>../librtmp2-server</code> checkout; use it only for source development.</p>
        ...
        <p><code>librtmp2</code> follows SemVer and remains on <code>0.x</code> during active development. For <code>0.x</code> releases, minor versions may include breaking API changes; <code>1.0.0</code> marks the first stable public API. Pin the crates.io release you tested if you depend on a specific API shape.</p>
        <p><code>librtmp2-server</code> and <code>librtmp2-server-panel</code> are also pre-1.0 &mdash; REST API shapes, environment variable names, and Docker images may evolve. Prefer GitHub Releases and image tags over hard-coding versions from this page.</p>
